...
def get_feats_multitab(feat_path, max_feat_count):
    feats_0_list = []
    feats_1_list = []
    lab_dict = {}
    ...
    for fil in os.listdir(feat_path):
        ...
        site_ind = int(mch.group(1))
        label = 1 if site_ind % 10 == 9 else 0
        feats1.append(label)
...
def run(feat_path, max_feat_count, num_test, vgg, batch_size, epochs, per_epoch):
    X_tr, y_tr, X_ts, y_ts = get_shuffled_multitab( max_feat_count, *get_instances_multitab( num_test, *get_feats_multitab(feat_path, max_feat_count) ) )
    ...
    if per_epoch:
        if epochs > 1:
            newl_tr, newa_tr, newl_ts, newa_ts = run(feat_path,  max_feat_count, num_test, vgg, batch_size, epochs-1, per_epoch)
...
def batch(feat_path, num_test, start_feat, end_feat, step_feat=100, vgg=False, batch_size=32, epochs=2, per_epoch=False):
    tr_loss = []; tr_acc = []; ts_loss = []; ts_acc = []
    for feat in xrange(start_feat, end_feat, step_feat):
        print "Starting run - max_feat_count = %d"%feat
        l_tr, a_tr, l_ts, a_ts = run(feat_path, feat, num_test, vgg, batch_size, epochs, per_epoch)
        tr_loss.append(l_tr)
        tr_acc.append(a_tr)
        ts_loss.append(l_ts)
        ts_acc.append(a_ts)
...
epochs=20
per_epoch=True
tr_loss, tr_acc, ts_loss, ts_acc = batch(feat_path, num_test, start_feat, end_feat, step_feat, vgg, batch_size, epochs, per_epoch)
...
print timestamp
pkl_file_name = 'results/nct_%s_%s_%s_%s_%s_%s_%s_%s.pkl'%(timestamp, start_feat, end_feat, step_feat, vgg, batch_size, epochs, per_epoch)
